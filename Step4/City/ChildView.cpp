...
///This is for the view history when its active
...
///This is for adding space rocket
void CChildView::OnSpaceRocket()
{
	// TODO: Add your command handler code here
	auto tile = make_shared<CTileLandscape>(&mCity);
	tile->SetImage(L"rocketB.png");
	tile->SetLocation(InitialX, InitialY);
	mCity.Add(tile);
	Invalidate();
}

///This is for loading rocket
void CChildView::OnSpaceLoad()
{
	auto tile = make_shared<CTileLandscape>(&mCity);
	tile->SetImage(L"rocket.png");
	tile->SetLocation(InitialX, InitialY);
	mCity.Add(tile);
	Invalidate();
}
