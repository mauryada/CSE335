...
* Constructor
*/
CCarpFish::CCarpFish(CAquarium *aquarium) : CItem(aquarium)
{
	mFishImage = unique_ptr<Bitmap>(Bitmap::FromFile(FishBetaImageName.c_str()));		///This declares which image is to be used
...
* \return true if hit.
*/
bool CCarpFish::HitTest(int x, int y)
{
...
///Returns true because its a predator.
bool CCarpFish::IsItPredator()
{
	return true;
}